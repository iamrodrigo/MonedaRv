...
contract("MonedaRv", function(accounts) {
	...
	it('Transfers ownership', function() {
		return MonedaRv.deployed().then(function(instance) {
			...
			assert.equal(balance.toNumber(), 1000000 - 250000);
		});
	});
	...
	it('approves tokens for delegated transfer', function() {
		return MonedaRv.deployed().then(function(instance) {
			tokenInstance = instance
			// Call doesn't send the info to the blockchain,
			// it only calls it in Solidity to get the return value
	...
	it('handles delegated token transfers', function() {
		...
		.catch(function(error) {
			...
			assert.equal(allowance.toNumber(), 0, 'deducts the amount from the allowance account');
		});
	});
});